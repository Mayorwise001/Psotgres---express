...
router.get('/', async (req, res) => {
  ...
  router.post('/new',  async (req, res) => {
    ...
    try {
      const queryText = 'INSERT INTO usersn (username) VALUES ($1) RETURNING *';
  ...
  router.get('/users', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM usersn ORDER BY id ASC');
  ...
  router.get('/delete', async (req, res) => {
    try {
        await pool.query('DELETE FROM usersn');
